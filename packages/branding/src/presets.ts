...
const PRO_PRESETS: readonly BrandingPreset[] = [
  {
    id: 'unicore-default',
...
export const BRANDING_PRESETS: readonly BrandingPreset[] = PRO_PRESETS;

/**
 * Look up a preset by ID. Returns undefined if not found.
 */
export function findPreset(id: string): BrandingPreset | undefined {
  const found = BRANDING_PRESETS.find((p) => p.id === id);
  if (found) return found;
  // Fall back to community preset lookup for forward compatibility
  const community = findCommunityPreset(id);
  if (!community) return undefined;
  return { ...community, config: { ...community.config } } as BrandingPreset;
}
