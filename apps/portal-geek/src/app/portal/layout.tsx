...
const navItems = [
  ...
  { href: '/portal/multiplayer', label: 'Multiplayer', icon: Users },
  { href: '/portal/downloads', label: 'Downloads', icon: Download },
  { href: '/portal/plugins', label: 'Plugins', icon: Puzzle },
    ...
    <aside className="flex h-full w-60 flex-col border-r border-zinc-800 bg-zinc-950">
      <div className="flex h-14 items-center justify-between border-b border-zinc-800 px-4">
        <Link href="/portal" className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-zinc-50">Geek Portal</span>
        </Link>
        {onClose && (
      ...
      <div className="border-t border-zinc-800 p-4">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="flex flex-1 items-center gap-2 rounded-md px-3 py-2 text-sm text-zinc-400 hover:bg-zinc-800 hover:text-zinc-50"
          >
...
export default function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem('geek_user');
      if (stored) {
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <header className="flex h-14 items-center justify-between border-b border-zinc-800 bg-zinc-950 px-4 lg:hidden">
          <div className="flex items-center">
            ...
            <span className="ml-3 text-sm font-bold text-zinc-50">Geek Portal</span>
          </div>
          <ThemeToggle />
        </header>
