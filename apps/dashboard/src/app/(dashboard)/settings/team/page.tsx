...
const ROLE_DESCRIPTIONS: Record<UserRole, string> = {
  ...
  [UserRole.Marketer]:
    "Dashboard, Growth Agent, Comms Agent — campaigns and outreach",
...
export default function SettingsTeamPage() {
  ...
  const handleInvite = useCallback(async () => {
    ...
    setIsInviting(true);
    try {
      const newMember = await api.post<TeamMember>("/settings/team/invite", {
        email: inviteEmail,
        role: inviteRole,
      });
  ...
  const handleChangeRole = useCallback(async (id: string, role: UserRole) => {
    try {
      await api.put(`/settings/team/${id}/role`, { role });
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            UniCore supports up to 5 team members (Community Edition).
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <Dialog open={inviteOpen} onOpenChange={setInviteOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Send an invitation email. The recipient will set their own
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            {(Object.values(UserRole) as UserRole[])
                              .filter(
                                (r) =>
                                  r !== UserRole.Owner && r !== member.role,
                              )
                              .map((role) => (
                                <DropdownMenuItem
                                  key={role}
