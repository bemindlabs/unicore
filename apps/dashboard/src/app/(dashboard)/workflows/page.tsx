...
/** Demo workflow definitions shown when the workflow service is unavailable. */
const DEMO_WORKFLOWS: WorkflowDefinition[] = [
  {
    id: 'wf-order-fulfillment',
    name: 'Order Fulfillment Pipeline',
    description: 'Automatically notify warehouse, update inventory, and email customer when order is confirmed',
    enabled: true,
    schemaVersion: 1,
    trigger: { type: 'erp.order.created' },
    actions: [
      { id: 'a1', type: 'call_agent', config: { agentName: 'inventory-agent', promptTemplate: 'Check stock availability for order {{payload.orderNumber}} with items: {{payload.items}}' } },
      { id: 'a2', type: 'send_notification', config: { channel: 'slack', recipient: '#warehouse', subject: 'New Order', bodyTemplate: 'Order {{payload.orderNumber}} received — {{payload.total}} {{payload.currency}}' } },
      { id: 'a3', type: 'update_erp', config: { entity: 'order', entityId: '{{payload.orderId}}', fields: { fulfillmentStatus: 'PICKING' } } },
    ],
  ...
  },
  {
    ...
    description: 'When inventory drops below reorder point, create purchase order and notify procurement team',
    enabled: true,
    schemaVersion: 1,
    trigger: { type: 'erp.inventory.low' },
    actions: [
      { id: 'a1', type: 'call_agent', config: { agentName: 'procurement-agent', promptTemplate: 'Generate purchase order for {{payload.productName}} (SKU: {{payload.sku}}). Current stock: {{payload.currentQty}}, reorder qty: {{payload.reorderQty}}' } },
      { id: 'a2', type: 'send_telegram', config: { chatId: '-1001234567890', message: '⚠️ Low stock: {{payload.productName}} — only {{payload.currentQty}} units left. PO created automatically.' } },
    ],
  ...
  },
  {
    ...
    description: 'Send payment reminder emails and escalate to finance team when invoices become overdue',
    enabled: true,
    schemaVersion: 1,
    trigger: { type: 'erp.invoice.overdue' },
    actions: [
      { id: 'a1', type: 'send_notification', config: { channel: 'email', recipient: '{{payload.contactEmail}}', subject: 'Payment Reminder: Invoice {{payload.invoiceNumber}}', bodyTemplate: 'Dear {{payload.contactName}},\n\nInvoice {{payload.invoiceNumber}} for {{payload.total}} {{payload.currency}} is now overdue. Please arrange payment at your earliest convenience.\n\nThank you.' } },
      { id: 'a2', type: 'send_notification', config: { channel: 'slack', recipient: '#finance', subject: 'Overdue Invoice Alert', bodyTemplate: '🔴 Invoice {{payload.invoiceNumber}} ({{payload.total}} {{payload.currency}}) for {{payload.contactName}} is overdue by {{payload.daysOverdue}} days.' } },
    ],
  ...
  },
  {
    ...
    description: 'When a new lead is captured, score them with AI and start automated nurture campaign',
    enabled: true,
    schemaVersion: 1,
    trigger: { type: 'event', event: 'crm.lead.created' },
    actions: [
      { id: 'a1', type: 'call_agent', config: { agentName: 'crm-agent', promptTemplate: 'Analyze lead {{payload.contactName}} from {{payload.company}}. Source: {{payload.source}}. Assign a lead score (0-100) and recommend next action.' } },
      { id: 'a2', type: 'update_erp', config: { entity: 'contact', entityId: '{{payload.contactId}}', fields: { leadStage: 'CONTACTED' } } },
      { id: 'a3', type: 'send_notification', config: { channel: 'email', recipient: '{{payload.contactEmail}}', subject: 'Welcome to UniCore!', bodyTemplate: 'Hi {{payload.contactName}},\n\nThank you for your interest in UniCore! Here are some resources to get you started...' } },
    ],
  ...
  },
  {
    ...
    description: 'Generate and send daily P&L summary, top orders, and key metrics every morning at 9 AM',
    enabled: true,
    schemaVersion: 1,
    trigger: { type: 'schedule.cron', cron: '0 9 * * *' },
    actions: [
      { id: 'a1', type: 'call_agent', config: { agentName: 'finance-agent', promptTemplate: 'Generate a daily business summary including: revenue, expenses, new orders, new leads, and any alerts for today.' } },
      { id: 'a2', type: 'send_telegram', config: { chatId: '-1009876543210', message: '📊 Daily Report\n\n{{steps.a1.output}}', parseMode: 'HTML' } },
    ],
  ...
  },
  {
    ...
    description: 'When invoice is paid, update order status, send receipt, and log to accounting channel',
    enabled: false,
    schemaVersion: 1,
    trigger: { type: 'erp.invoice.paid' },
    actions: [
      { id: 'a1', type: 'send_notification', config: { channel: 'email', recipient: '{{payload.contactEmail}}', subject: 'Payment Received — {{payload.invoiceNumber}}', bodyTemplate: 'Thank you for your payment of {{payload.total}} {{payload.currency}}. Your invoice {{payload.invoiceNumber}} has been marked as paid.' } },
      { id: 'a2', type: 'send_notification', config: { channel: 'slack', recipient: '#accounting', subject: 'Payment Received', bodyTemplate: '💰 Payment received: {{payload.invoiceNumber}} — {{payload.total}} {{payload.currency}} from {{payload.contactName}}' } },
    ],
  ...
  },
  {
    ...
    actions: [
      { id: 'a1', type: 'call_agent', config: { agentName: 'sales-agent', promptTemplate: 'Create onboarding plan for new customer {{payload.contactName}} ({{payload.company}}). Include: welcome email, demo scheduling, and documentation links.' } },
      { id: 'a2', type: 'send_line', config: { to: '{{payload.lineUserId}}', message: 'Welcome aboard, {{payload.contactName}}! 🎉 Your UniCore account is ready.' } },
    ],
  ...
  },
  {
    ...
    trigger: { type: 'manual' },
    actions: [
      { id: 'a1', type: 'call_agent', config: { agentName: 'ops-agent', promptTemplate: 'Export all contacts updated in the last 24 hours and sync to external CRM via API.' } },
      { id: 'a2', type: 'send_notification', config: { channel: 'slack', recipient: '#ops', subject: 'Data Sync Complete', bodyTemplate: 'CRM sync completed: {{steps.a1.output}}' } },
    ],
