...
import type { BackofficeAgent, AgentStatus, RoomId } from '@/lib/backoffice/types';
import { PixelAvatar } from './PixelAvatar';

interface Props {
  agent?: BackofficeAgent;
  mode: 'edit' | 'add';
  onSave: (agent: BackofficeAgent) => void;
  onDelete?: () => void;
  onClose: () => void;
}
...
const COLORS = [
  '#00e5ff', '#4fc3f7', '#ce93d8', '#81c784', '#ef5350',
  '#7c4dff', '#ffb74d', '#4dd0e1', '#f06292', '#aed581',
];

export function AgentModal({ agent, mode, onSave, onDelete, onClose }: Props) {
  const [form, setForm] = useState<BackofficeAgent>(
    agent || { id: '', name: '', role: '', status: 'idle', room: 'standalone', activity: '', color: '#00e5ff', deskItems: [] }
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      ...
      <div className="relative w-full max-w-md mx-4 border border-cyan-900/50 bg-[#0a0e1a] overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-cyan-900/30 bg-[#0d1225]">
          ...
          <button onClick={onClose} className="text-cyan-600/50 hover:text-cyan-400 text-lg">&times;</button>
        </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            <label className="font-mono text-[9px] text-cyan-600/60 block mb-1.5 uppercase tracking-wider">Agent Name</label>
            <input
              value={form.name}
          ...
          <div>
            <label className="font-mono text-[9px] text-cyan-600/60 block mb-1.5 uppercase tracking-wider">Role</label>
            <input
              value={form.role}
          ...
          <div>
            <label className="font-mono text-[9px] text-cyan-600/60 block mb-1.5 uppercase tracking-wider">Activity</label>
            <input
              value={form.activity || ''}
          ...
          <div>
            <label className="font-mono text-[9px] text-cyan-600/60 block mb-1.5 uppercase tracking-wider">Status</label>
          ...
          <div>
            <label className="font-mono text-[9px] text-cyan-600/60 block mb-1.5 uppercase tracking-wider">Room</label>
            <select
              value={form.room}
          ...
          <div>
            <label className="font-mono text-[9px] text-cyan-600/60 block mb-1.5 uppercase tracking-wider">Color</label>
              ...
              {COLORS.map(c => (
                <button
                  ...
                  className={`w-6 h-6 border-2 transition-all ${
                    form.color === c ? 'border-white scale-110 shadow-[0_0_8px_rgba(255,255,255,0.3)]' : 'border-transparent hover:border-white/30'
                  }`}
            ...
            {mode === 'edit' && onDelete && (
              <button
                ...
                className={`font-mono text-[10px] border py-2.5 px-4 transition-all uppercase tracking-wider ${
                  confirmDelete ? 'bg-red-500/20 border-red-500/50 text-red-400' : 'bg-red-500/5 border-red-900/30 text-red-500/60 hover:border-red-500/40'
                }`}
              >
