...
class TestAgent extends SpecialistAgentBase {
  ...
  readonly config: SpecialistAgentConfig = {
    ...
    description: 'Minimal agent for base-class tests',
    version: '0.0.0',
  };
  ...
  protected async executeTool(call: ToolCall, _context: AgentContext): Promise<ToolResult> {
    if (call.toolName === 'test_tool') {
      return { toolName: call.toolName, result: { echo: call.arguments['input'] } };
    }
...
function makeContext(overrides: Partial<AgentContext> = {}): AgentContext {
  return {
    sessionId: 'sess-1',
    history: [],
    businessContext: { businessName: 'ACME Corp' },
    ...overrides,
  };
}
    ...
    it('should embed available tools in response data', async () => {
      ...
      expect(response.data?.['availableTools']).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ name: 'test_tool', description: 'A test tool' }),
        ]),
      );
    });

    it('should catch errors and return an error AgentResponse', async () => {
      // Patch executeInternal to throw
      jest
        .spyOn(agent as unknown as { executeInternal: () => Promise<unknown> }, 'executeInternal')
