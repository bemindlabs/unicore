 ...
 * provider reload — see TokenTrackingService.reloadPricingOverrides().
 *
...
export class TokenTrackingService implements OnModuleInit {
   ...
   * follow the same shape as DEFAULT_PROVIDER_COSTS.  Only the entries
   * present in the override object are replaced — all other defaults remain.
   *
   * This method is called automatically on module init and can also be
   * triggered externally via POST /api/v1/llm/reload (which delegates to
   * LlmService.reloadProviders → ProviderFactoryService.reloadProviders).
   * To wire the reload of pricing into the provider reload, call this method
   * from TokenTrackingService after reloading providers.
   */
  async reloadPricingOverrides(): Promise<void> {
    ...
    try {
      const res = await fetch(
        `${gatewayUrl}/api/v1/settings/ai-config/keys`,
        {
      ...
      if (!raw) {
        this.logger.debug('No llmPricingOverrides found in settings — using defaults');
        return;
      }
  ...
  private parsePricingOverrides(raw: unknown): ProviderPricingMap {
    ...
    for (const [providerId, models] of Object.entries(raw as Record<string, unknown>)) {
      if (typeof models !== 'object' || models === null || Array.isArray(models)) {
        this.logger.warn(`Pricing override for provider "${providerId}" is malformed — skipping`);
        continue;
      }

      result[providerId] = {};

      for (const [modelId, pricing] of Object.entries(models as Record<string, unknown>)) {
        if (
          typeof pricing !== 'object' ||
          pricing === null ||
          typeof (pricing as Record<string, unknown>)['inputPer1M'] !== 'number' ||
          typeof (pricing as Record<string, unknown>)['outputPer1M'] !== 'number'
        ) {
          this.logger.warn(
