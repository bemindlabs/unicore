...
export class ContactsService {
  ...
  async create(dto: CreateContactDto): Promise<ContactRecord> {
    if (dto.email) {
      const ex = await this.prisma.contact.findUnique({ where: { email: dto.email } });
      if (ex) throw new ConflictException(`A contact with email ${dto.email} already exists`);
    }
    ...
    this.logger.log(`Contact created: ${c.id}`);
    return c;
  }

  async findAll(q: QueryContactsDto): Promise<PaginatedResult<ContactRecord>> {
    const { page = 1, limit = 20, search, type, minLeadScore } = q;
  ...
  async update(id: string, dto: UpdateContactDto): Promise<ContactRecord> {
    ...
    if (dto.email) {
      const ex = await this.prisma.contact.findFirst({ where: { email: dto.email, NOT: { id } } });
      if (ex) throw new ConflictException(`A contact with email ${dto.email} already exists`);
    }
  ...
  async getTopLeads(minScore: number, limit = 20): Promise<ContactRecord[]> {
    return this.prisma.contact.findMany({
      ...
      orderBy: { leadScore: 'desc' }, take: limit,
    });
  }
}
