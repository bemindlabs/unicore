...
export class EventPublisherService implements OnModuleInit, OnModuleDestroy {
  ...
  async publish<T = unknown>(topic: ErpTopic, payload: T, key?: string): Promise<void> {
    if (!this.ready) { this.logger.warn(`Kafka not ready - dropping event "${topic}"`); return; }
    ...
    try {
      this.kafkaClient.emit(topic, {
        key: key ?? envelope.eventId, value: JSON.stringify(envelope),
        headers: { 'x-event-id': envelope.eventId, 'x-event-type': topic, 'x-schema-version': String(envelope.schemaVersion), 'x-source': envelope.source },
      });
