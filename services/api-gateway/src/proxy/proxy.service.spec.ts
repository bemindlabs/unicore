...
function createFakeResponse(opts: { statusCode?: number; headers?: Record<string, string> } = {}) {
  const emitter = new EventEmitter() as EventEmitter & { statusCode: number; headers: Record<string, string> };
...
function createFakeRequest() {
  const emitter = new EventEmitter() as EventEmitter & { write: jest.Mock; end: jest.Mock; destroy: jest.Mock };
...
describe('ProxyService', () => {
  ...
  describe('forward', () => {
    it('throws NotFoundException when no service matches the path', async () => {
      await expect(
        service.forward({ method: 'GET', path: '/unknown/resource', headers: {}, body: null }),
      ).rejects.toThrow(NotFoundException);
    });

    it('throws NotFoundException for path that only partially matches prefix', async () => {
      await expect(
        service.forward({ method: 'GET', path: '/erp-extra/resource', headers: {}, body: null }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('proxies successfully for /erp/* path', async () => {
      ...
      const result = await service.forward({
        method: 'GET', path: '/erp/invoices', headers: { host: 'localhost:4000' }, body: null,
      });
    ...
    it('throws BadGatewayException on connection error', async () => {
      ...
      await expect(
        service.forward({ method: 'GET', path: '/erp/invoices', headers: {}, body: null }),
    ...
    it('injects x-user-id header when userId is provided', async () => {
      ...
      await service.forward({ method: 'GET', path: '/ai/chat', headers: {}, body: null, userId: 'user-abc-123' });
    ...
    it('routes /ai/* to port 4200', async () => {
      ...
      await service.forward({ method: 'POST', path: '/ai/generate', headers: {}, body: Buffer.from('{}') });
    ...
    it('routes /rag/* to port 4300', async () => {
      ...
      await service.forward({ method: 'GET', path: '/rag/query', headers: {}, body: null });
    ...
    it('routes /bootstrap/* to port 4500', async () => {
      ...
      await service.forward({ method: 'POST', path: '/bootstrap/setup', headers: {}, body: null });
    ...
    it('strips the service prefix from the downstream path', async () => {
      ...
      await service.forward({ method: 'GET', path: '/erp/invoices/123', headers: {}, body: null });
